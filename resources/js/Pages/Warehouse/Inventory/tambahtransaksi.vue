<template lang="">
    <blankVue>
        ...
        <template v-slot:konten>
            <form
                class="w-full h-full"
                @submit.prevent="form.post(route('warehouseinventorytransaksi.store'))"
            >
                <h3 class="font-bold text-lg mb-1 capitalize">Nama</h3>
                <select
                    ...
                    required
                >
                    <option :value="item.id" v-for="item in inventory">
                        {{ item.nama }}
                    </option>
                </select>
                <h3 class="font-bold text-lg mb-1">jumlah</h3>
                <input
                    required
                    type="num"
                    v-model="form.jumlah"
