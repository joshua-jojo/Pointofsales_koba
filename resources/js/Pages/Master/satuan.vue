    ...
    <blankVue>
        ...
        <template v-slot:modals>
            <div class="modal" id="modal-tambah">
                <div class="modal-box">
                    <h2 class="font-bold text-lg">Tambah Satuan</h2>
                    ...
                    <div class="modal-action">
                        <button class="btn bg-green-500 btn-square loading" v-if="formtambah.processing"></button>
                        <a
                            v-else
            ...
            <div class="modal" id="modal-edit">
                <div class="modal-box">
                    <h2 class="font-bold text-lg">Edit Satuan</h2>
                    <div class="form-control">
                        <label class="label">
                            <span class="label-text">Nama</span>
                        </label>
                        <input
                            ...
                            class="input input-bordered"
                        />
                        <span class="label-text" v-if="errors.nama">{{
                            errors.nama
                        }}</span>
                    </div>
                    <div class="modal-action">
                        <button class="btn bg-green-500 btn-square loading" v-if="formedit.processing"></button>
                        <a
                            v-else
                            class="btn bg-green-500 hover:bg-green-400 text-white border-0"
                            @click="submitedit"
            ...
            <div class="modal" id="modal-hapus">
                <div class="modal-box">
                    ...
                    <div class="modal-action">
                        <button class="btn bg-red-500 btn-square loading" v-if="formhapus.processing"></button>
                        <a
                            v-else
...
export default {
    ...
    setup() {
        ...
        function submitedit() {
            formedit.put(
                route("mastersatuan.update", { satuan: formedit.id }),
                {
                    onSuccess: () => {
                        formedit.reset(), klik();
                    },
        ...
        function submithapus() {
            formhapus.delete(
                route("mastersatuan.update", { satuan: formhapus.id }),
                {
                    onSuccess: () => {
                        formedit.reset(), klik();
                    },
    ...
    computed: {
        filteredItems() {
            return this.satuan.filter((item) => {
                return (
                    item.nama
                        .toLowerCase()
